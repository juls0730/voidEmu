...
bool **allocate_framebuffer(int width, int height) {
    ...
    bool **fb = (bool **)calloc(height, sizeof(bool *));
    if (fb == NULL) {
        return NULL;
    }

    bool *pixel_array = (bool *)calloc(width * height, sizeof(bool));
    if (pixel_array == NULL) {
        return NULL;
    }
...
class Chip8 {
  public:
    Chip8(char *rom_path) {
        ...
        printf("Reading file: %s for %ld bytes\n", rom_path, rom_size);

        rom = (uint8_t *)calloc(rom_size, sizeof(uint8_t));
        if (rom == NULL) {
            fprintf(stderr, "Failed to allocate memory!\n");
            exit(1);
        }

        int err = read(rom_fd, rom, rom_size);
        if (err < 0) {
        ...
        stack = (uint16_t *)calloc(16, sizeof(uint16_t));
        if (stack == NULL) {
    ...
    void reset() {
        ...
        // no need to manually reallocate the framebuffer, since it will be
        // dealt with in the render thread
  ...
  private:
    ...
    static constexpr auto cycle_time =
        std::chrono::milliseconds(TARGET_MS_PER_CYCLE);
};
...
void draw(SDL_Renderer *renderer, SDL_Texture *texture, Chip8 *chip8) {
    if (pixels_array == NULL ||
        ...
        pixels_array =
            (uint32_t *)calloc(SCREEN_HEIGHT * SCREEN_WIDTH, sizeof(uint32_t));
        if (pixels_array == NULL) {
...
void timer_thread(Chip8 *chip8) {
    ...
    while (chip8->running.load(std::memory_order::seq_cst)) {
...
uint8_t *Chip8::step() {
    uint16_t opcode = (read_mem(pc) << 8) | read_mem(pc + 1);
    if (!is_executable(pc)) {
    ...
    printf("PC: 0x%04x OP: 0x%04x\n           INSTRUCTION_TYPE: %d\n", pc,
           opcode, bytecode.instruction_type);

    pc += 2;

    switch (bytecode.instruction_type) {
    case HLT: {
        // From the Octo interpreter. Halt the emulator.
        while (true) {
            // spin
        }
    }
...
// interpreters.
    ...
    case LOAD_BYTE: {
        ...
        this->v[bytecode.operand.byte_reg.reg] = bytecode.operand.byte_reg.byte;
        break;
    }
    case ADD_BYTE: {
        //             Set Vx = Vx + kk.
...
int Chip8::run() {
    ...
    std::thread input(input_thread, this);
    std::thread timer(timer_thread, this);

    // this value is higher than the max for uint8_t, so a program can never
    // return 0x100, so we can use this value as an indicator that the emulator
    // was restarted, but didnt exit
    int ret = 0xFF;
    this->running.store(true);

    while (this->running.load(std::memory_order::seq_cst)) {
        if (this->should_reset.load(std::memory_order::seq_cst)) {
            printf("Resetting emulator\n");
            this->should_reset.store(false);
            this->reset();
        ...
        auto elapsed_time = duration_cast<std::chrono::milliseconds>(
            std::chrono::high_resolution_clock::now() - start_time);
        if (elapsed_time < cycle_time) {
            printf("%ld", cycle_time.count() - elapsed_time.count());
